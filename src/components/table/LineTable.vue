...
export default {
    ...
    data:function() {
        return {
            data_info:[20,21,22,23,24,50,40,50,-10]
        }
    },
    methods:{
        draw_table_body:function() {
            ...
            //表明纵坐标刻度
            cxt.fillText("温度(℃)",10,30)
            for(let i = 0;i < 10;i++) {
                cxt.fillText(""+(50-i*10),20,55+i*30)
            }
            //爷拿来画横坐标
            for(let i =0;i<10;i++) {
                let date = new Date()
        ...
        draw_data:function() {
            ...
            for(let i = 0;i<data_list.length;i++) {
                ...
                let data = -((parseInt(data_list[i].temperature) + 50)+20*((parseInt(data_list[i].temperature)+50)/10))  //y=(x+50)+20*((x+50)/10) 映射关系
                switch(i) {
                    case 0:
