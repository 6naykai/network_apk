<template>
    <div class="list_body">
        ...
        <input class="upload" type="button" v-if="login_now" value="上传信息" @click="upload_file"/>
    </div>
</template>
...
export default{
    ...
    beforeMount() {
        ...
        axios.post("/ccr/query/"+sessionStorage.getItem('username'))
            .then((response) => {
            let file_list = [];
            console.log(response.data)
            for (let i = 0; i < response.data.data.length; i++) {
